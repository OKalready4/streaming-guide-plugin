...
class Streaming_Guide_Seasonal_Generator extends Streaming_Guide_Base_Generator {
    ...
    private function get_seasonal_movies($provider_id, $season_config) {
        try {
            $movies = $this->tmdb->discover_movies([
                ...
                'watch_region' => 'US',
                'with_genres' => implode('|', $season_config['genres']),
            ...
            if (is_wp_error($movies)) {
                $this->log_error('TMDB API error getting seasonal movies', [
                    'provider_id' => $provider_id,
                    'season' => $season_config['name'],
            ...
            if (!isset($movies['results'])) {
                $this->log_error('Invalid TMDB response for seasonal movies', [
                    'provider_id' => $provider_id,
                    'season' => $season_config['name'],
                    'response' => $movies
                ]);
            ...
            $seasonal_movies = array();
            foreach ($movies['results'] as $movie) {
                if ($this->is_content_on_platform($movie['id'], 'movie', $provider_id)) {
                    $movie['media_type'] = 'movie';
                    $movie['seasonal_score'] = $this->calculate_seasonal_score($movie, $season_config);
                    $seasonal_movies[] = $movie;
                }
            }
            
            return $seasonal_movies;
            
        } catch (Exception $e) {
            $this->log_error('Error getting seasonal movies', [
                'provider_id' => $provider_id,
                'season' => $season_config['name'],
                'error' => $e->getMessage()
            ]);
    ...
    /**
     * Get seasonal TV shows from TMDB
     */
    private function get_seasonal_tv($provider_id, $season_config) {
        try {
            ...
            if (is_wp_error($tv_shows)) {
                $this->log_error('TMDB API error getting seasonal TV shows', [
                    'provider_id' => $provider_id,
                    'season' => $season_config['name'],
            ...
            if (!isset($tv_shows['results'])) {
                $this->log_error('Invalid TMDB response for seasonal TV shows', [
                    'provider_id' => $provider_id,
                    'season' => $season_config['name'],
                    'response' => $tv_shows
                ]);
            ...
            $seasonal_shows = array();
            foreach ($tv_shows['results'] as $show) {
                if ($this->is_content_on_platform($show['id'], 'tv', $provider_id)) {
                    $show['media_type'] = 'tv';
                    $show['seasonal_score'] = $this->calculate_seasonal_score($show, $season_config);
                    $seasonal_shows[] = $show;
                }
            }
            
            return $seasonal_shows;
            
        } catch (Exception $e) {
            $this->log_error('Error getting seasonal TV shows', [
                'provider_id' => $provider_id,
                'season' => $season_config['name'],
                'error' => $e->getMessage()
            ]);
    ...
    private function create_seasonal_title($platform_name, $season_config, $content_type) {
        ...
        $title_templates = array(
            "Best {$season_name} Content on {$platform_name}: Curated for the Season",
            "{$season_name} Streaming Guide: Perfect Picks for {$platform_name}",
            "{$platform_name}'s Ultimate {$season_name} Watchlist",
            "Cozy Up with These {$season_name} Picks on {$platform_name}",
            "{$season_name} Essentials: What to Watch on {$platform_name}",
            "Your {$season_name} Streaming Companion on {$platform_name}",
            "{$platform_name}'s {$season_name} Collection: Mood-Perfect Entertainment",
            "Seasonal Streaming: {$season_name}'s Best on {$platform_name}"
        );
    ...
    /**
     * Get seasonal reason for content
     */
    private function get_seasonal_reason($item, $season_config) {
        $reasons = array();
        
        // Check genres against season preferences
        if (!empty($item['genre_ids'])) {
            $matching_genres = array_intersect($item['genre_ids'], $season_config['genres']);
            if (!empty($matching_genres)) {
                $reasons[] = "Perfect for " . strtolower($season_config['name']) . " viewing";
            }
        }
        
        // Check keywords in overview
        if (!empty($item['overview'])) {
            $overview_lower = strtolower($item['overview']);
            foreach ($season_config['keywords'] as $keyword) {
                if (strpos($overview_lower, $keyword) !== false) {
                    $reasons[] = "Captures the spirit of " . strtolower($season_config['name']);
                    break;
                }
            }
        }
        ...
        $reasons[] = "Matches the " . $season_config['mood'] . " mood of " . strtolower($season_config['name']);
