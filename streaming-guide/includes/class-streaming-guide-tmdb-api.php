...
class Streaming_Guide_TMDB_API {
    ...
    public function make_request($endpoint, $params = array()) {
        ...
        if (is_wp_error($response)) {
            $this->error_handler->log_api_error('TMDB', $endpoint, $response->get_error_message(), $params);
            return $response;
        }

        // Check for API-level errors (e.g., 401, 404)
    ...
    public function get_watch_providers($content_id, $media_type) {
        return $this->make_request("/{$media_type}/{$content_id}/watch/providers");
    }
