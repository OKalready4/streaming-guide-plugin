...
class Streaming_Guide_Cron_Handler {
    ...
    public function run_weekly_generation() {
        ...
        foreach ($platforms as $platform) {
            ...
            try {
                // Start generation
                $generation_id = $this->state_manager->start_generation('weekly', $platform, array());
                
                if ($generation_id) {
                    // Schedule background processing with slight delay
                    wp_schedule_single_event(
                        time() + ($generated * 30), // 30 second delay between each
                ...
                $this->error_handler->log_error(
                    "Failed to schedule weekly generation for {$platform}",
        ...
        $this->error_handler->log_error(
            "Completed weekly generation scheduling",
            array('scheduled' => $generated, 'failed' => $failed),
            'info'
        );
    }
    ...
    public function run_monthly_generation() {
        ...
        foreach ($platforms as $platform) {
            ...
            try {
                // Start generation
                $generation_id = $this->state_manager->start_generation('monthly', $platform, array());
                ...
                $failed++;
                $this->error_handler->log_error(
                    "Failed to schedule monthly generation for {$platform}",
                    array('error' => $e->getMessage()),
                    'error'
                );
            }
        ...
        $this->error_handler->log_error(
            "Completed monthly generation scheduling",
            array('scheduled' => $generated, 'failed' => $failed),
            'info'
        );
    }
