...
class Streaming_Guide_TMDB_Attribution {
    ...
    public static function add_frontend_attribution($content) {
        ...
        // Only add attribution to streaming guide posts
        if (!class_exists('Streaming_Guide_Attribution_Helper') || !Streaming_Guide_Attribution_Helper::requires_attribution($post->ID)) {
            return $content;
        }
        
        // Get the attribution HTML
        $attribution = self::get_frontend_attribution();
        
        // Add attribution after the content
        return $content . $attribution;
    }
    
    /**
     * Get frontend attribution HTML
     */
    private static function get_frontend_attribution() {
        $logo_url = self::get_tmdb_logo('small');
        
        $html = '<div class="tmdb-attribution">';
        $html .= '<div class="tmdb-attribution-content">';
        $html .= '<img src="' . esc_url($logo_url) . '" alt="TMDB Logo" class="tmdb-attribution-logo">';
        $html .= '<div class="tmdb-attribution-text">';
        $html .= '<p>This product uses the TMDB API but is not endorsed or certified by TMDB.</p>';
        $html .= '<p>All movie and TV show information, including titles, descriptions, images, and metadata, is provided by <a href="https://www.themoviedb.org/" target="_blank" rel="noopener">The Movie Database (TMDB)</a>.</p>';
        $html .= '</div>';
        $html .= '</div>';
        $html .= '</div>';
