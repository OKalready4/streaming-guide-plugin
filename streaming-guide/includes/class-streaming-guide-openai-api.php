...
class Streaming_Guide_OpenAI_API {
    ...
    public function make_request($messages, $temperature = 0.7, $max_tokens = 4000) {
        if (empty($this->api_key)) {
            $this->error_handler->log_api_error('OpenAI', 'chat/completions', 'OpenAI API key not configured');
        ...
        if (is_wp_error($response)) {
            ...
            if (strpos($error_message, 'cURL error 28') !== false) {
                $this->error_handler->log_api_error('OpenAI', 'chat/completions', 'Request timed out');
                return new WP_Error('timeout', 'OpenAI API request timed out');
            }
            
            if (strpos($error_message, 'SSL certificate') !== false) {
                $this->error_handler->log_api_error('OpenAI', 'chat/completions', 'SSL certificate verification failed');
                return new WP_Error('ssl_error', 'OpenAI API SSL certificate verification failed');
            }
            
            $this->error_handler->log_api_error('OpenAI', 'chat/completions', $error_message);
            return $response;
        }
        ...
        if ($status_code !== 200) {
            ...
            if ($status_code === 429) {
                ...
                $this->error_handler->log_api_error('OpenAI', 'chat/completions', "Rate limit exceeded. Retry after: {$retry_after}");
                return new WP_Error('rate_limit', 'OpenAI API rate limit exceeded', ['retry_after' => $retry_after]);
            }
            
            // Handle authentication errors
            if ($status_code === 401) {
                $this->error_handler->log_api_error('OpenAI', 'chat/completions', 'Invalid API key');
                return new WP_Error('auth_error', 'Invalid OpenAI API key');
            }
            
            $this->error_handler->log_api_error('OpenAI', 'chat/completions', "{$error_message} (Status: {$status_code})", [], $error_data);
        ...
        if (json_last_error() !== JSON_ERROR_NONE) {
            $this->error_handler->log_api_error('OpenAI', 'chat/completions', 'Failed to parse JSON response', [], $body);
            return new WP_Error('json_error', 'Failed to parse OpenAI response');
        }
        
        if (!isset($data['choices'][0]['message']['content'])) {
            $this->error_handler->log_api_error('OpenAI', 'chat/completions', 'Invalid response format', [], $data);
