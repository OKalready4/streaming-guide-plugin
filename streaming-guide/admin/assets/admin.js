...
jQuery(document).ready(function($) {
    ...
    $('.generate-btn').on('click', function(e) {
        ...
        // Make AJAX request
        if (streamingGuideAdmin.debug) {
            console.log('Debug Info:', {
                nonce: streamingGuideAdmin.streaming_guide_nonce,
                nonceAction: streamingGuideAdmin.nonce_action,
                nonceField: streamingGuideAdmin.nonce_field,
                ajaxurl: streamingGuideAdmin.ajaxurl,
                fullData: {
                    action: 'streaming_guide_generate',
                    type: generatorType,
                    platform: platform,
                    [streamingGuideAdmin.nonce_field]: streamingGuideAdmin.streaming_guide_nonce
                }
            });
        }
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    let errorMessage = response.data.message || 'Generation failed.';
                    if (response.data.debug && streamingGuideAdmin.debug) {
                        errorMessage += ' (' + response.data.debug + ')';
                    }
    ...
    function checkGenerationStatus() {
        ...
        $.ajax({
            ...
            data: {
                ...
                streaming_guide_nonce: streamingGuideAdmin.streaming_guide_nonce
            },
            success: function(response) {
